...
void osarpopt(instanceStat *inst, vector<nodeStat> &nodeVec, double **mdist, probStat* problem, nodeArcsStruct *nas, solStats *sStat){
//Original SARP model (Li et al 2014)

//Creating environment and model 
// 	char var[100];
// 	IloEnv env;
// 	IloModel model(env, "mioSARP");

//     int sizeReq = inst->n + 2*inst->m - 1;
//     int fDepot = inst->n + 2*inst->m;
//     int lDepot = inst->n + 2*inst->m + inst-> K - 1;
//     int fDummy = inst->n + 2*inst->m + inst->K;
//     int lDummy = inst->n + 2*inst->m + 2*inst->K - 1;
...
//     IloArray <IloBoolVarArray> x(env, sizeReq);

//     for (int i = 0; i < sizeReq; i++){
//         x[i] = IloBoolVarArray (env, sizeReq);
//         for(int j = 0; j < sizeReq; ++j){
...
//         }
//     }
//     //New vars ykc (1 if c is the first customer of vehicle k)

// 	IloArray <IloBoolVarArray> y(env, inst->K);

//     for (int k = 0; k < inst->K; k++){
//         y[k] = IloBoolVarArray (env, sizeReq);
//         for(int i = 0; i < sizeReq; ++i){
//             if (nas->arcV[i][j] != true){//if the vehicle K can serve i
//                 continue; // If arc i to j is invalid
//             } 
//             sprintf(var, "y(%d,%d)", k, i);
//             y[k][i].setName(var);
//             model.add(y[k][i]);
//         }
//     }

//     //New vars pi (p_i = 1 if node i is visited; 0 cc)

// 	IloBoolVarArray p(env, nodeVec.size()); 
