...
void mipbundle(instanceStat *inst, vector<nodeStat> &nodeVec, double **mdist, bundleStat *bStat, clSt *cStat, probStat* problem, solStats *sStat){
    ...
    IloNumVarArray u(env, inst->K, 9, inst->T);
    ...
    IloNumVarArray uf(env, inst->K, 9, inst->T);
...
void mipbundlep(instanceStat *inst, vector<nodeStat> &nodeVec, double **mdist, bundleStat *bStat, clSt *cStat, probStat* problem, solStats *sStat){

	//MIP
	//Creating environment and model 
	char var[100];
	IloEnv env;
	IloModel model(env, "bpSARP");
	int setN;
    int setP;//with passengers
    int csetN;
    int csetP; //with passengers
...
//	//Creating objective function
	...
	for (int a = 0; a < bStat->bArcVec.size(); a++){
        ...
        for (int k1 = 0; k1 < bStat->arcV[u][v].size(); k1++){
            int k = bStat->arcV[u][v][k1];
            objFunction -= (double)(inst->costkm*mdist[i][j]) * x[u][v][k];
            // cout << "Cost: " << inst->costkm*mdist[i][j] << endl;
            // getchar();
        }
...
//    //////////////////////////////////////
    ...
    int threads = 1;
	IloCplex bpSARP(model);
	bpSARP.exportModel("bpSARP.lp");
	bpSARP.setParam(IloCplex::Threads, threads);

    bpSARP.setParam(IloCplex::Param::TimeLimit, 7200);
//    // bSARP.setParam(IloCplex::Param::TimeLimit, 10);

    IloNum start;
    IloNum time;
    start = bpSARP.getTime();
//    //bSARP.setOut(env.getNullStream());
	bpSARP.solve();
    time = (bpSARP.getTime() - start)/threads;

	cout << "\nSol status: " << bpSARP.getStatus() << endl;
	sStat->feasible = bpSARP.isPrimalFeasible();
...
//    // cout << " Tree_Size: " <<  bSARP.getNnodes() + bSARP.getNnodesLeft() + 1 << endl;
	...
	if(sStat->feasible){
        ...
        solStatIni(sStat);
		cout << "\nObj Val: " << setprecision(15) << bpSARP.getObjValue() << endl;

		sStat->solprofit = bpSARP.getObjValue();
        sStat->solDual = bpSARP.getBestObjValue();
        sStat->time = time;

        if (((bpSARP.getBestObjValue() - bpSARP.getObjValue())/bpSARP.getBestObjValue()) * 100 < 0.01) {
			sStat->status = "Optimal";
		} else {
			sStat->status = "Feasible";
		}
		...
		for (int i = 0; i < bStat->bundleVec.size(); i++){
			for(int j = 0; j < bStat->bundleVec.size(); ++j){
				if (bStat->bArcs[i][j] == true){
                    for (int k1 = 0; k1 < bStat->arcV[i][j].size(); k1++){
                        int k = bStat->arcV[i][j][k1];
						if (bpSARP.getValue(x[i][j][k]) > 0.5){
							auxPair.first = i;
							auxPair.second = j;
							sStat->solvec[k].push_back(auxPair);
							// cout << i << " " << j << " " << k << ": " << bpSARP.getValue(x[i][j][k]) << endl;
							// getchar();
						}
					}
				}
...
void fipbundle(instanceStat *inst, vector<nodeStat> &nodeVec, double **mdist, bundleStat *bStat, clSt *cStat, probStat* problem, solStats *sStat, fipBundleStats *fipStat){
	...
	for (int k = 0; k < fipStat->solPass.size(); k++){
        ...
        b[k] = IloNumVarArray (env, bStat->bundleVec.size(), 9, inst->T);
    ...
    IloNumVarArray s(env, sizeofs, 9, inst->T);
