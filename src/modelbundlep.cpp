...
void makeBundlesPartial (instanceStat *inst, vector<nodeStat> &nodeVec, bundleStat *bStat, bundlePStat *bPStat,
                        clSt *cStat, clPSt *cPStat, vector< vector<bParcelStruct> > &clsParcel,
...
void orderBundles(instanceStat *inst, bundleStat *bStat, vector< vector<int> > &fullsorted, clSt *cStat){
    ...
    for (int i = 0; i < inst->n; i++){
        //for(int k = 0; k < cStat->clusterVec[i].size() - 2*inst->K; k++){
            ...
            for (int j = counter; j < counter + (inst->m*3 + 1); ++j) {
            ...
            for (int j = 0; j < paired.size(); ++j) {                
                ...
                sortedBundles.push_back(paired[j].second);
        ...
        //}
        fullsorted.push_back(sortedBundles);
        sortedBundles.clear();
        sortedProf.clear();
        ...
        //}
        //cout << "]" << endl;
        counter = (inst->m*3 + 1)*(i + 1);

    }

    for (int i = 0; i < fullsorted.size(); i++){
        cout << "Cluster " << i << ": ";
        for (int j = 0; j < fullsorted[i].size(); j++){
            cout << fullsorted[i][j] << " ";
        }
        cout << endl;
    }
...
//    pair<int, int> bFArc;
//    int auxK;
//    //cout << "1. arc between 64 and 32: " << bStat->bArcs[64][32] << endl;
...
void bundleMethodPartial(nodeStat *node, instanceStat *inst, double **mdist, vector<nodeStat> &nodeVec, probStat* problem, solStats *sStat){
    ...
    ////makeBundlesPartial(inst, nodeVec, &bStat, &bPStat, &cStat, clsParcel, problem);
    ...
    ////bestProfBundles(inst, &bStat, &cStat);

    //orderBundles(inst, &bStat, fullsorted, &cStat);

    //makeBundlesPartial(inst, nodeVec, &bStat, &bPStat, &cStat, &cPStat, clsParcel, problem, fullsorted);


    //cout << "\nBundle Profit: " << endl;
    //for (int i = 0; i < bPStat.bundleProfVec.size(); i++){
    //    cout << i << ": " << setw(3) << std:: right << bPStat.bundleProfVec[i] << endl;
    //}
    //cout << endl;

    //for (int i = 0; i < cPStat.clusterVec.size(); i++){
    //    cout << "\nCluster " << i << ": [";
    //    for(int k = 0; k < cPStat.clusterVec[i].size(); k++){
    //        cout << "(" << cPStat.clusterVec[i][k] << ") " << "[";
    //        for (int j = 0; j < bPStat.bundleVec[cPStat.clusterVec[i][k]].size(); j++){
    //            cout << setw(3) << std:: right << bPStat.bundleVec[cPStat.clusterVec[i][k]][j];
    //            if (j < bPStat.bundleVec[cPStat.clusterVec[i][k]].size() - 1){
    //                cout << ",";
    //            }
    //            else{
    //                cout << "] ";
