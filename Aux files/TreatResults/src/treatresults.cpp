...
void createOutputInst(vector<instInfoN>& vecInst, string instName)
{
    ...
    for(int i = 0; i < vecInst.size(); i++){
        if(vecInst[i].name == firstInst){
            ...
            ofile << "Instance,n,m,served,K,Sol Time,Sol Val" << endl;
        }
        ofile << vecInst[i].name << "," << vecInst[i].n << "," << vecInst[i].m << "," << vecInst[i].servP << "," << vecInst[i].K << "," << vecInst[i].soltime << "," << vecInst[i].objval << endl;
    } 
}
	...
	if(in.is_open()) {
	    ...
	    while(getline(in, line)) {
            ...
            if (optflag){
                ...
                flag = substrPosition(line, "Solving Time");
                ...
                if (flag) {
                    loc = line.find_first_of(":");
                    inst.objval = stoi(line.substr(loc + 1, line.size() - loc - 1));
