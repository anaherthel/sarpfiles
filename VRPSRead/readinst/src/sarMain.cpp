...
void runInstance(int argc, char** argv) //No BaPcode, no parameters
{
    // SarData data(applSpecParam, instanceName);
    string instanceName = argv[1];
    SarData data(instanceName);//no parameters
...
//   BcInitialisation bapcodInitPtr(argc, argv, "");
  ...
  cout << "Application version is " << APPLICATION_VERSION << ", " << APPLICATION_DATE << endl;
  cout << "command line:";
