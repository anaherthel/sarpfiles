...
Graph::Graph(int n, int m, int d, vector<InputVertex>& input_vertices)
{
	...
	for(int i = 0; i < n; i++)
	{
		double service_time = 2*custpick[i]->_service_time + distance(*custpick[i], *custdel[i])/41.0;
		...
		for(auto& vj : _V)
		{
			if(vi._id != vj._id)
			{
				...
				else
				{
					...
					_cost[make_pair(vi._id,vj._id)] = 0.46*distance(*vout, *vin);
					if(vi._type == Parcel && vi._demand > 0)
						_cost[make_pair(vi._id,vj._id)] += get_parcel_cost(vi._id - n - d, parcelpick, parceldel);
					_time[make_pair(vi._id,vj._id)] = distance(*vout, *vin)/41.0 + vi._service_time;
				}    
			}
		}
	}
	...
	_fixed_cost = -3.24*n;
	for(int i = 0; i < n; i++)
		_fixed_cost += (0.46 - 1.03)*distance(*custpick[i], *custdel[i]);
...
void SarData::readCSarpData(string path_file)
{
	...
	_G = Graph(_n, _m, _d, _input_vertices);
	fclose(file);
}
/*******************************************************************/
// Vertex
